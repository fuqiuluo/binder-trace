...
def gen_compile_commands(cmd_file_search_path, out_dir):
    print("正在收集 *.o.cmd 文件列表...", file=sys.stderr)
    ...
    if not cmd_files:
        print("没有找到 *.o.cmd 文件:", ", ".join(cmd_file_search_path), file=sys.stderr)
        return

    print("正在解析 *.o.cmd 文件...", file=sys.stderr)
    ...
    print("正在写入 compile_commands.json...", file=sys.stderr)
...
def main():
    ...
    cmd_parser.add_argument('-O', '--out-dir', type=str, default=os.getcwd(), help="构建输出目录")
    cmd_parser.add_argument('cmd_file_search_path', nargs='*', help="*.cmd 文件搜索路径")
