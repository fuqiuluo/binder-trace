...
无子命令运行时，`binder-trace` 会连接内核模块提供的 socket 事件流，开启 Binder
事务采集，并持续将请求和回复写入 JSONL，适合脚本处理或保存:
...
受当前 ABI 限制，事件暂时无法区分 `binder`、`hwbinder` 和 `vndbinder`，因此
`binder_device` 固定为 `unknown`。该字段与 `/dev/binder` 的路径形式无关，使用
binderfs 不会影响事件采集。
